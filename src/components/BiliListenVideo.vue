...
import { omissionText } from '../utils'
import 'vue3-toastify/dist/index.css'

const audioListStore = useAudioListStore()
audioListStore.getAudioList()
const audioList = audioListStore.audioList
const bvid = ref('')
const audioUrl = ref('')
...
<template>
  <div class="h-full bg-cover bg-center" :style="styleObject">
    <div class="w-full h-full flex items-center justify-center flex-col backdrop-blur-sm">
      <div class="container p-6 bg-white border border-gray-200 rounded-lg shadow">
        ...
        <div>
          ...
          <ul class="divide-y divide-gray-200">
            <li v-for="(audio, index) in audioList" :key="audio.cid" class="flex flex-row justify-between items-center p-2">
              <div class="flex flex-row items-center">
                <span class="inline-flex items-center px-2 py-1 text-sm font-medium text-blue-800 bg-blue-100 rounded">
                  {{ index + 1 }}
                </span>
                <span class="ml-2 text-gray-900 text-sm">{{ omissionText(audio.name, 50) }}</span>
              </div>
              <div class="flex flex-row items-center gap-2">
                <button type="button" class="text-blue-700 border border-blue-700 hover:bg-blue-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center" @click="() => handlePlay(audio, index)">
                ...
                <button type="button" class="text-blue-700 border border-blue-700 hover:bg-blue-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center" @click="() => handleDeleteAudio(index)">
                  <svg class="w-4 h-4 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18 17.94 6M18 18 6.06 6" />
                  </svg>
                </button>
                <button type="button" class="text-blue-700 border border-blue-700 hover:bg-blue-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center" @click="() => handleOpenLink(audio)">
                  <svg class="w-4 h-4 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
